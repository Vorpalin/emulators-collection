...
uint16_t CPU65::absIndexed(uint32_t &cycles, uint8_t idx, bool write) {
  ...
  if (write || (base & 0xFF00) != (addr & 0xFF00)) ++cycles;
...
void CPU65::branch(uint32_t &cycles, bool cond) {
  ...
  if (cond) {
    ...
    PC += off;
    ++cycles;
    if ((old ^ PC) & 0xFF00) ++cycles;
  }
}

uint8_t CPU65::getStatus(bool breakFlag) const {
  return (static_cast<uint8_t>(N) << 7) | (static_cast<uint8_t>(V) << 6) |
         (1u << 5) | (static_cast<uint8_t>(breakFlag) << 4) |
         (static_cast<uint8_t>(D) << 3) | (static_cast<uint8_t>(I) << 2) |
         (static_cast<uint8_t>(Z) << 1) | static_cast<uint8_t>(C);
}
...
void CPU65::axs(uint8_t value) {
  ...
  uint16_t result = static_cast<uint16_t>(ax) - static_cast<uint16_t>(value);
...
uint32_t CPU65::execute() {
  ...
  switch (instruction) {
    ...
    case INS_JSR:  // JSR (Jump to Subroutine)
    {
      uint16_t addr = fetch16(cycles);
      uint16_t returnAddr =
          PC - 1;  // Address of the next instruction after JSR
      ++cycles;    // Increment cycles for the operation
    ...
    {
      uint8_t status = (N << 7) | (V << 6) | (1 << 5) | (B << 4) | (D << 3) |
                       (I << 2) | (Z << 1) | C;
      push(cycles, status);
      ++cycles;  // Increment cycles for the operation
      break;
    ...
    case INS_RTS:  // RTS (Return from Subroutine)
    {
      ...
      uint8_t highByte = pull(cycles);
      PC = ((highByte << 8) | lowByte) +
           1;    // Return to the instruction after JSR
      ++cycles;  // Increment cycles for the operation
      ++cycles;  // Increment cycles for the operation
      ++cycles;  // Increment cycles for the operation
      break;
    ...
    default: {
      std::cerr << "Unknown instruction: " << std::hex
                << static_cast<int>(instruction) << std::endl;
      break;
    }
  }
  return cycles;
